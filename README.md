...
>> 실행코드는 압축파일로 첨부합니다.(sm_project) 두 기능 모두 같은 코드안에 있지만 현재 아직 원활한 화면 이동을 지원하지 않아(다른 사람들의 코드와 병합 후 추가 예정) 두 기능을 보시려면 Manifest에서 시작화면을 각각 설정해줘야합니다. 이 점은 근시일내로 화면전환 기능을 추가할 예정입니다.
>
>1. 태그 기능 추가
>
